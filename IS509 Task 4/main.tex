...
In Task 3, we explored the Agent-to-Agent (A2A) communication framework. 
...
This gap raises critical questions.
...
\begin{enumerate}
  ...
  \item [For whom does it work?](Comparative/relational) Does the effectiveness of the typed semantic layer differ between MAS using small language models ($<$14B parameters) versus foundation models ($>$70B parameters) in terms of protocol adherence and negotiation success rates?
\end{enumerate}
...
This evidence would allow to further explore the specific semantic constructs that contribute to this improvement.
...
\paragraph{Hypothesis:} Deterministic task delegation and role negotiation in LLM-based multi-agent systems emerge when agents share a typed interaction protocol. If they support (1) a shared ontology of task types, (2) well-formed truth-conditional commitments, and (3) role-dependent inference rules, then agents can parse messages into unambiguous semantic actions.
...
\end{enumerate}
\paragraph{Explanation:} The study investigates whether LLM agents achieve more predictable coordination when their communication is constrained by explicit semantic formalisms. Without such constraints, agents rely on natural-language inference, which introduces ambiguity and stochastic interpretation. By adding elements such as type systems, role predicates, and truth-conditional commitments, agent messages become machine-interpretable semantic acts instead of free-form text. If determinism increases with stronger semantic structure, it would support the hypothesis that LLM-multi-agent systems require lightweight formal semantics to achieve reliable delegation and negotiation.
...
\paragraph{Research Question:} Does the effectiveness of the typed semantic layer differ between MAS using small language models ($<$14B parameters) versus foundation models ($>$70B parameters) in terms of protocol adherence and negotiation success rates?
...
\paragraph{IV:} Model size category (small language models $<$14B parameters, foundation models $>$70B parameters).
...
\newpage
\subsection{Methodology (extending the assignment scope, AI-generated. Just to get an idea of what to write next)}
