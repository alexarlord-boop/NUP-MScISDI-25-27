...
\subparagraph{}
Figure \ref{fig:semantic_gap} illustrates the communication landscape in multi-agent systems. Traditional formal protocols (FIPA-ACL, KQML) offered high semantic precision and verifiability but suffered from rigidity and poor adoption. Modern LLM-based agents communicate through natural language prompts and role assignments, providing flexibility and adaptability but introducing ambiguity and non-determinism. Current standards like A2A provide syntactic infrastructure (message envelopes, routing, discovery) but lack semantic guarantees, leaving a critical gap. Our research targets this gap by proposing a typed semantic layer that bridges formal verifiability with LLM flexibility.
...
\subparagraph{}
This research addresses a critical gap in the emerging standard for agent-to-agent communication at a pivotal moment in its adoption trajectory. 
...
\subparagraph{Research scope.}
This research develops a typed semantic extension for A2A protocol, implementing it as a Message.metadata schema with formal performatives and interaction contracts. The extension will be declared in AgentCard.capabilities.extensions (Section 8) and validated at message send/receive boundaries. Evaluation focuses on multi-turn task coordination, delegation reliability, and negotiation determinism using LLM-based agents in controlled environments.

\subparagraph{Protocol boundaries.}
The semantic layer operates within A2A's extension mechanism (Section 4.6), targeting Message and Task objects only. Client-to-agent communication (MCP protocol) and human-agent interaction are excluded. The work focuses on horizontal agent-to-agent communication using A2A's three standard protocol bindings (JSON-RPC, gRPC, HTTP+JSON).

\subparagraph{Implementation constraints.}
Experiments use simulated multi-agent environments rather than distributed production deployments. LLM computational requirements limit experimental scale to 3-5 concurrent agents. Security implementation (Section 13), cross-organization authentication, and push notification mechanisms (Section 3.5.3) are acknowledged but not evaluated.

\subparagraph{Evaluation scope.}
Semantic layer tested on representative task types: delegation, negotiation, and multi-step coordination. Full coverage of A2A's task lifecycle states (Section 4.1.3) and all possible agent skill combinations is beyond scope. Baseline comparison uses natural-language-only A2A communication without semantic constraints.

\subparagraph{Rationale.}
Focusing on the extension mechanism ensures compatibility with A2A's design philosophy and immediate applicability to existing implementations. Simulated environments provide reproducibility and controlled variables essential for validating semantic guarantees. The subset approach enables completion within research timeframe while demonstrating generalizability to broader agent interaction patterns.

\newpage
\subsection{Methodology overview}

\subsubsection{LLM selection strategy}
\subparagraph{Selection criteria.}
LLMs evaluated based on: (1) instruction-following capability for structured output, (2) context window size for multi-turn interactions, (3) API availability and cost, (4) reasoning capability for semantic constraint interpretation.

\subparagraph{Candidate models.}
Primary evaluation uses GPT-4o (OpenAI) for strong reasoning and Claude 3.5 Sonnet (Anthropic) for long-context handling. Comparative baseline includes GPT-3.5-turbo for cost-effectiveness analysis. Model selection (will be) validated through preliminary tests on semantic constraint parsing accuracy.
\textit{Probably small language models will be a better target due to inherited obvious reasoning limitations.}
...
\hline
\rowcolor{gray!20}
\textbf{Dimension} & \textbf{Variable} & \textbf{Levels} & \textbf{Metrics} \\ \hline
\textbf{Communication} & Semantic layer & With / Without & Task success rate \\ 
\textbf{Mode} & & (semantic vs. NLP) & Negotiation consistency \\ \hline
\textbf{Task} & Complexity & Simple, Multi-step, & Completion time \\ 
\textbf{Complexity} & & Negotiation-heavy & Message count \\ \hline
\textbf{Agent} & Model & GPT-4o, Claude 3.5, & Interpretation accuracy \\ 
\textbf{Configuration} & & Mixed combinations & Error rate \\ \hline
\textbf{Interaction} & Turns & 2, 5, 10+ & Semantic drift \\ 
\textbf{Depth} & & & Context coherence \\ \hline
\end{tabular}
\end{table}
...
\begin{tikzpicture}[
  ...
  decision/.style={diamond, draw, fill=orange!10, text width=2.5cm, align=center, aspect=2},
  arrow/.style={->, >=stealth, thick}
]

% Nodes
\node[box] (baseline) {Baseline: Natural Language A2A};
\node[box, below of=baseline] (semantic) {Semantic Layer A2A};
\node[box, right=2cm of baseline] (task1) {Task Type 1: Delegation};
\node[box, below of=task1] (task2) {Task Type 2: Negotiation};
\node[box, below of=task2] (task3) {Task Type 3: Multi-step};
\node[box, right=2cm of task2] (metrics) {Collect Metrics: Success, Consistency, Time};
\node[box, below of=metrics] (analysis) {Comparative Analysis};

% Arrows
\draw[arrow] (baseline) -- (task1);
\draw[arrow] (baseline) -- (task2);
\draw[arrow] (baseline) -- (task3);
\draw[arrow] (semantic) -- (task1);
\draw[arrow] (semantic) -- (task2);
\draw[arrow] (semantic) -- (task3);
\draw[arrow] (task1) -- (metrics);
\draw[arrow] (task2) -- (metrics);
\draw[arrow] (task3) -- (metrics);
\draw[arrow] (metrics) -- (analysis);

\end{tikzpicture}
\caption{Experimental workflow comparing semantic vs. natural language communication modes}
\label{fig:evaluation_workflow}
\end{figure}

\subparagraph{Controlled variables.}
All experiments use identical A2A infrastructure (JSON-RPC binding), consistent task prompts, and fixed agent role definitions. LLM temperature set to 0.1 for reproducibility. Each experimental condition repeated minimum 10 times to establish statistical significance.
\textit{Is 10 times enough? More models to include in a swarm?}
\subparagraph{Success criteria.}
Semantic layer considered successful if: (1) task completion rate improves $>$15\% vs. baseline, (2) negotiation consistency $>$90\%, (3) semantic drift reduced in 10+ turn interactions, (4) no degradation in completion time $>$20\%.
