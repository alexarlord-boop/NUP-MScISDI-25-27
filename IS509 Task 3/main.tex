...
These systems can revolutionize industries by automating workflows and optimizing decision-making \cite{Chen2024PromiseOfMultiAgentAI}.
Businesses and individuals can leverage LLMs to build autonomous multi-agent systems with frameworks, such as AutoGPT,
CrewAI, and Langchain \cite{Sinha2025RiseOfMAS} \cite{Sinha2025BuildingMultiAgent}.
...
FIPA Agent Communication Language (ACL) defines a set of communicative acts and message structures for agent communication \cite{inbook}. 
KQML (Knowledge Query and Manipulation Language) is another early standard that provides a framework for knowledge sharing and querying among agents \cite{Finin1994KQML}.
...
With ACP’s recent merger into Google’s A2A under the Linux Foundation, A2A is rapidly becoming the dominant open standard for agent-to-agent communication \cite{LinuxFoundation2025_A2A}. 
...
It defines message envelopes, routing, agent metadata, capability discovery, and extensible JSON-based interaction primitives \cite{A2A2025}.
...
This semantic gap motivates research into typed, non-NLP communication layers that can enable more reliable, verifiable, and deterministic multi-agent behavior - exploring guarantees that agents understands each other beyond message structure.
...
Knowledge gaps: How can agents communicate deterministically and reliably without relying on natural-language prompts? Existing protocols (MCP, OpenAI Agents SDK) address tool or in-process communication, and historical protocols (KQML, FIPA-ACL) either failed to scale or are obsolete. We aim to build communication layers on top of A2A that add semantic guarantees, improving reliability and verifiability in MAS.
...
\subparagraph{}
Structured, semantically precise communication in multi-agent systems is increasingly critical as agentic AI applications scale across industries.
