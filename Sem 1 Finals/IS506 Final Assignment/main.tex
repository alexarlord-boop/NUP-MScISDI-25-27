...
\begin{itemize}
    \item \textbf{Data Preprocessing:} Clean, normalize, and aggregate multi-source data within QGIS environment
    \item \textbf{Feature Engineering:} Extract predictive variables directly from geospatial layers (elevation, proximity, land use)
    \item \textbf{Model Types (Transparent \& Auditable):}
    \begin{itemize}
        \item Rule-based decision trees for flood risk assessment
        \item Linear regression for traffic congestion prediction
        \item Constraint satisfaction for resource allocation
        \item Monte Carlo simulations for scenario planning
    \end{itemize}
    \item \textbf{Explainability:} SHAP values and decision tree visualizations for public sector accountability
    \item \textbf{Validation:} Government stakeholder review process for model acceptance
\end{itemize}

\subsubsection{"Decision Theater" Visualization}
\begin{itemize}
    \item \textbf{Blender 3D Engine:} High-fidelity cinematic rendering for stakeholder presentations
    \item \textbf{QGIS-to-Blender Pipeline:} Automated export of geospatial data to 3D scenes
    \item \textbf{Scenario Simulation:} "What-if" visualizations for policy impact assessment
    \item \textbf{CesiumJS Operational View:} Lightweight, browser-based daily monitoring
    \item \textbf{Data Layer Toggle:} Real-time switching between infrastructure, environmental, and social data
    \item \textbf{Temporal Controls:} Playback historical events and simulate future scenarios
\end{itemize}
...
\textbf{Phase 1 (Months 1-6): TerraSync Foundation}
\begin{itemize}
    \item QGIS + PostGIS core setup
    \item 5 essential n8n workflows (weather, IoT, government databases)
    \item Basic Blender visualization templates
    \item Single territory deployment (Cyprus pilot)
\end{itemize}

\textbf{Phase 2 (Months 7-12): Decision Theater}
\begin{itemize}
    \item Advanced Blender "Decision Theater" capabilities
    \item 15+ data source integrations
    \item CesiumJS web interface for daily operations
    \item Multi-territory support (3 countries)
    \item Explainable AI model library
\end{itemize}

\textbf{Phase 3 (Year 2): Governance \& Scale}
\begin{itemize}
    \item 30+ government-certified data workflows
    \item White-label municipal deployment packages
    \item Mobile applications for field workers
    \item Open-source community ecosystem (workflow marketplace)
\end{itemize}
