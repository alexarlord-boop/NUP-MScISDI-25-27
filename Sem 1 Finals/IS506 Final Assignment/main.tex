...
\subsection{Solution Overview: The "Open Canvas" Platform}
...
\begin{tikzpicture}[scale=0.9, transform shape, font=\sffamily]
    ...
    \fill[green!10] (0,0) rectangle (14,6);
    \draw[gray!40, dashed] (7,0) -- (7,6); % Border
    \node[rotate=90, gray, font=\bfseries] at (6.8, 3) {BORDER};
    
    \node[font=\bfseries\large, gray] at (3.5, 5.5) {Country A (Montenegro)};
    \node[font=\bfseries\large, gray] at (10.5, 5.5) {Country B (Albania)};

    % Proposed Route
    \draw[orange, line width=0.2cm] (1,1) to[out=30,in=200] (5,3) to[out=20,in=180] (9,2) to[out=0,in=150] (13,4);
    \node[orange!80!black, font=\bfseries] at (2, 1.5) {Proposed Highway};

    % Protected Zone (Conflict)
    \fill[green!40, opacity=0.6] (6, 1.5) circle (1.5);
    \node[green!30!black, font=\bfseries, align=center] at (6, 1.5) {Protected\\Nature Zone};
    \node[red, font=\bfseries] at (6, 2.5) {CONFLICT DETECTED};
    \draw[red, thick, ->] (6, 2.3) -- (6, 1.8);

    % Blender View Insert (Decision Theater)
    \draw[fill=white, drop shadow, thick] (8, 3.5) rectangle (13.5, 5.8);
    \node[anchor=north west, font=\bfseries\small] at (8.1, 5.7) {Blender "Decision Theater" View};
    % 3D Mockup inside
    \fill[cyan!20] (8.2, 3.7) rectangle (13.3, 5.3); % Sky
    \fill[green!30] (8.2, 3.7) -- (9, 4.5) -- (10, 4) -- (11, 4.8) -- (12, 4.2) -- (13.3, 3.7) -- cycle; % Mountains
    \draw[gray!80, line width=0.1cm] (8.5, 3.7) to[out=45,in=220] (13, 4.5); % Road in 3D
    \node[font=\tiny, align=center] at (10.5, 5.0) {Visual Impact Simulation:\\Route Option B (Tunnel)};

    % Data Sources
    \node[draw, fill=white, rounded corners] at (2, 4.5) {GIS Data A};
    \node[draw, fill=white, rounded corners] at (12, 1) {GIS Data B};
    \draw[->, dashed] (2, 4.2) -- (4, 3);
    \draw[->, dashed] (12, 1.3) -- (10, 2);

\end{tikzpicture}
\caption{Use Case 2 Visualization: Cross-Border Strategic Planning. The platform integrates disparate GIS data from two nations (background) and uses Blender (inset) to visualize environmental conflicts and simulate alternative routes for stakeholder consensus.}
\label{fig:balkan_corridor}
\end{figure}
...
\end{tikzpicture}
\caption{Use Case 1 Visualization: The TerraSync Operational Dashboard. The interface integrates real-time GIS visualization (center) with live n8n data streams (right) and predictive analytics (bottom) to support rapid decision-making.}
...
\begin{tikzpicture}[scale=0.9, transform shape, font=\sffamily]
    % Background Map
    \fill[gray!10] (0,0) rectangle (14,7);
    \draw[gray!30, step=1] (0,0) grid (14,7);
    \node[anchor=north west, text=gray] at (0.2,6.8) {GIS Terrain Layer: Flood Risk Analysis};

    % River
    \draw[blue!40, line width=1.5cm, opacity=0.5] (0,2) sin (4,3) cos (8,1) sin (12,2) cos (14,1.5);
    \draw[blue!60, line width=0.2cm] (0,2) sin (4,3) cos (8,1) sin (12,2) cos (14,1.5); % Core flow
    \node[blue!60, font=\itshape] at (2, 2.5) {River Bed};

    % Flood Zones (Heatmap)
    \fill[red, opacity=0.3] (6,0) rectangle (10,3); % Critical Zone
    \node[red!80!black, font=\bfseries] at (8, 0.5) {CRITICAL ZONE};
    
    \fill[orange, opacity=0.3] (4,2) rectangle (6,4); % Warning Zone
    \fill[orange, opacity=0.3] (10,1) rectangle (12,3);

    % City Blocks
    \foreach \x in {1,2,3, 11,12,13} {
        \fill[gray!60] (\x, 4.5) rectangle (\x+0.6, 5.1);
        \fill[gray!60] (\x, 5.5) rectangle (\x+0.6, 6.1);
    }
    \node[font=\small] at (2.5, 6.5) {Residential Area (Safe)};
ite] (1,8.5) rectangle (16,9.2);
    \node[anchor=west] at (1.2, 8.85) {\textbf{Limassol Port Authority} | Live Monitoring};
    \fill[green] (15, 8.85) circle (0.1);
    \node[anchor=east, font=\footnotesize] at (14.8, 8.85) {System Online};
