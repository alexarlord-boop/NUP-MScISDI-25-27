...
\begin{tikzpicture}[
    ...
    widebox/.style={rectangle, draw, thick, text width=6.6cm, minimum height=4cm, align=left, font=\small},
...
\node[widebox, fill=purple!10, below=6.0cm of kp.south west, anchor=north west] (cost) {
    \textbf{Cost Structure}\\[0.2cm]
    \textbf{Fixed:} Team (€30K/mo), Cloud (€5K/mo), Data licenses (€10K/mo), Admin (€5K/mo)\\
    \textbf{Variable:} Implementation (€20-40K), Sales (15-20\% revenue), Custom dev (€10-30K)\\
    \textbf{Drivers:} Data licensing, cloud scaling, talent, long sales cycles
};

\node[widebox, fill=teal!10, below=6.0cm of cs.south east, anchor=north east] (revenue) {
    \textbf{Revenue Streams}\\[0.2cm]
    • SaaS Licensing (€20-50K/mo) - 60\%\\
    • Implementation (€100-300K) - 25\%\\
    • Custom Integration (€50-150K) - 10\%\\
    • Analytics Services (€30-80K)\\
    • Support \& Training (€10-25K/yr) - 5\%
};
